...
GLImageSampler::GLImageSampler()
{
    ...
    samplerInfo.maxAnisotropy = GLFoundation::glDevice->deviceProperties.limits.maxSamplerAnisotropy;
    ...
    if (vkCreateSampler(GLFoundation::glDevice->device, &samplerInfo, nullptr, &imageSampler) != VK_SUCCESS)
        throw std::runtime_error("创建图片采样器失败！");
}
GLImageSampler::~GLImageSampler()
{
    vkDestroySampler(GLFoundation::glDevice->device, imageSampler, nullptr);
}
