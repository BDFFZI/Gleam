...
GLMeshLayout::GLMeshLayout(const GLMeshVertexLayout& vertexLayout, const VkPrimitiveTopology indexLayout)
{
    ...
    bindingDescription.stride = vertexLayout.vertexSize; //每段数据的大小

    attributeDescriptions.resize(vertexLayout.vertexAttributes.size());
    for (uint32_t i = 0; i < attributeDescriptions.size(); i++)
    {
        VkVertexInputAttributeDescription attributeDescription;
        attributeDescription.binding = 0; //管道的顶点缓冲区允许绑定多个，我们只绑定一个，对应索引为0
        attributeDescription.location = vertexLayout.vertexAttributes[i].location; //对应在shader中的顶点属性槽位
        attributeDescription.format = vertexLayout.vertexAttributes[i].format;
        attributeDescription.offset = vertexLayout.vertexAttributes[i].offset;
