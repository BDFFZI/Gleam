...
#include "GleamGL/Runtime/GLSwapChain.h"
...
public:
    GLTester()
    {
        ...
        glDescriptorPool = std::make_unique<
            GLDescriptorPool>(*descriptorSetLayout, static_cast<int>(maxFramesInFGleam));
        glDescriptorSets.resize(maxFramesInFGleam);
        for (size_t i = 0; i < maxFramesInFGleam; ++i)
        {
        ...
        //创建命令缓冲区
        glCommandBuffers.resize(maxFramesInFGleam);
        for (size_t i = 0; i < maxFramesInFGleam; ++i)
    ...
    void RecreateSwapChain()
    {
        ...
        maxFramesInFGleam = glSwapChain->images.size();
    ...
    {
        //创建帧缓冲区
        glFramebuffers.resize(maxFramesInFGleam);
        for (size_t i = 0; i < maxFramesInFGleam; ++i)
        {
