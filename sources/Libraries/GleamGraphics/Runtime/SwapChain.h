...
namespace Gleam
{
    ...
    public:
         ...
         * 向交换链申请开始一段呈现操作。
         *
         * 该函数会使系统向交互链申请呈现目标并提供下次呈现所需的命令缓冲区。
         * 若交换链失效，则会尝试进行重建，若无法重建，返回false，但为了兼容性，仍会提供原计划用于本次呈现的命令缓冲区，不过使用后不会有任何效果。
         *
         * 要进行呈现必须先调用该函数。因为在调用该函数之前，交换链数据是过期的，不应被使用！
         * （不过你可能会发现，即使违背该点，画面似乎也能显示正常，那是因为你一直抓着呈现执行的间歇在向当前屏幕上正使用的图片写入，这可能导致画面撕裂，而且等真正的呈现完成后，画面将变成黑屏)
         *
