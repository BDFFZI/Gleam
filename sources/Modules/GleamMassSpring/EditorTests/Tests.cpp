...
class TestSystem : public System<>
{
    ...
    {
        World::GetEntityAllocator().GetComponent<Particle>(entity).drag = 1;
    }

    void Start() override
    {
        GlobalMassSpringRenderingSystem->SetIsEnabled(true);
        Physics::SetGravity(float3{0.0f, -9.81f, 0.0f} * 2);
        ...
        //弹簧
        {
            Entity pointA = Physics::AddParticle(float3{-10, 0, 20});
            Entity pointB = Physics::AddParticle(float3{-15, 0, 20}, 0);

            FixedParticle(pointA);

            Physics::AddSpring(pointA, pointB);
        }
        ...
        //软绳
        {
            ...
            for (int i = 0; i < std::size(points); i++)
                points[i] = Physics::AddParticle(float3(15 + i, 0, 0));
            FixedParticle(points[0]);
            for (int i = 0; i < std::size(points) - 1; i++)
            {
                Physics::AddSpring(points[i], points[i + 1], 1);
            }
        }
        ...
        //布料
        {
            ...
            for (int i = 0, y = 0; i < size; i++, y += 1)
                for (int j = 0, x = 0; j < size; j++, x += 1)
                    points[i][j] = Physics::AddParticle(float3(15 + x, y, 10));
            ...
            for (int y = 0; y < size; y++)
                for (int x = 0; x < size; x++)
                {
                    //抗拉伸结构
                    if (x + 1 < size)
                        Physics::AddSpring(points[y][x], points[y][x + 1]);
                    if (y + 1 < size)
                        Physics::AddSpring(points[y][x], points[y + 1][x]);
                    //抗斜向拉伸结构
                    if (x + 1 < size && y + 1 < size) //右下
                        Physics::AddSpring(points[y][x], points[y + 1][x + 1]);
                    if (x + 1 < size && y - 1 >= 0) //右上
                        Physics::AddSpring(points[y][x], points[y - 1][x + 1]);
                    //抗弯折弹簧
                    if (x + 2 < size)
                        Physics::AddSpring(points[y][x], points[y][x + 2]);
                    if (y + 2 < size)
                        Physics::AddSpring(points[y][x], points[y + 2][x]);
                }
        }
...
#ifdef GleamEngineEditor
Gleam_MakeEditorSystem(TestSystem)
#else
Gleam_MakeRuntimeSystem(TestSystem)
#endif
