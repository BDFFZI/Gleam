...
namespace Gleam
{
        ...
        Particle& particle)
    {
        ...
        if (equal(dot(deformationDirection, movementDirection), 0.0f)) //初始在碰撞内或完全在表面滑动
            constrainedPosition = particle.position + worldOffset;
        else //撞击碰撞体，回到质点撞击前所在的碰撞表面
            constrainedPosition = Geometry::Intersects(
                Plane{particle.position + worldOffset, deformationDirection},
    ...
    void CollisionSystem::Update()
    {
        View<LocalToWorld, WorldToLocal, Cuboid, Collider>::Each(
            ...
            {
                View<Particle>::Each(
                    [&localToWorld,&worldToLocal,&cuboid,&collider](Particle& particle)
                    {
                        ComputeCollision(localToWorld, worldToLocal, cuboid, collider, particle);
                    }
                );
            }
        );
        ...
        View<LocalToWorld, WorldToLocal, Sphere, Collider>::Each(
            ...
            {
                View<Particle>::Each(
                    [&localToWorld,&worldToLocal,&sphere,&collider](Particle& particle)
                    {
                        ComputeCollision(localToWorld, worldToLocal, sphere, collider, particle);
                    }
                );
            }
        );
