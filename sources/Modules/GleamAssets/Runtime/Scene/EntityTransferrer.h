...
namespace Gleam
{
    ...
    struct FieldDataTransferrer_Transfer<Entity>
    {
        static void Invoke(FieldDataTransferrer& serializer, Entity& value)
        {
            ...
            AssetRef assetRef = AssetBundle::GetAssetRef(assetPtr).value_or(GetAssetRef(&value));
            assert(value == Entity::Null || !assetRef.assetBundleID.is_nil() && "引用的实体未被资源化！");
            //序列化或反序列化引用信息
            serializer.Transfer(assetRef);
            SetAssetRef(&value, assetRef); //存储指针对应的引用信息，以供第二次指针遍历时依然能拿到指针引用
            //重新定位资源地址
    ...
    private:
        ...
        static AssetRef GetAssetRef(void* ptr)
        {
            return pointerMapping[ptr];
        }
        static void SetAssetRef(void* ptr, const AssetRef& value)
        {
            pointerMapping[ptr] = value;
            std::cout << ptr <<":" << to_string(value) << std::endl;
        }
    };
}